(() => {
  ...
  const embeddedCardArt = globalScope.embeddedCardArt instanceof Map ? globalScope.embeddedCardArt : new Map();

  function getEmbeddedFullArt(id) {
    const art = embeddedCardArt.get(id);
    return typeof art === "string" ? art : "";
  }

  const ancientTreantFullArt = getEmbeddedFullArt("ancient-treant");
  const embercapShamanFullArt = getEmbeddedFullArt("embercap-shaman");
  const petalWardenFullArt = getEmbeddedFullArt("petal-warden");
  ...
  const cardIllustrations = new Map([
    ...
    ["petal-warden", {
      image: petalWardenFullArt,
      fullCard: petalWardenFullArt.length > 0,
    }],
    ["glimmer-fox", { icon: "🦊" }],
    ["embercap-shaman", {
      image: embercapShamanFullArt,
      fullCard: embercapShamanFullArt.length > 0,
    }],
    ["thornborn-stalker", { icon: "🌿" }],
    ["luminous-hart", { icon: "🦌" }],
    ["spirit-of-canopy", { icon: "🌲" }],
    ["ancient-treant", {
      image: ancientTreantFullArt,
      fullCard: ancientTreantFullArt.length > 0,
    }],
  ]);

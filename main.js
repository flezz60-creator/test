(() => {
  ...
  allItems.forEach((item) => {
    if (!cardsBySet.has(item.setId)) {
      cardsBySet.set(item.setId, []);
    }
    cardsBySet.get(item.setId).push(item);
  });
  sets.forEach((set) => {
    const entries = cardsBySet.get(set.id) ?? [];
  ...
  const duelProfiles = {
    ...
    "embercap-shaman": {
      text: "Schamane: Entfacht rare Jagdenergie und stärkt Verbündete mit seinem Embercap-Zauber.",
    },
    "druid-of-the-grove": {
    ...
    "petal-warden": {
      abilities: ["bulwark"],
      text: "Bollwerk: Die Warden hält Angriffe auf und schützt den Hüter mit floralen Lichtschranken.",
    },
    "wolfkin-alpha": {
  ...
  function setupPanelSlider() {
    ...
    if (!panelResizeObserver && typeof ResizeObserver !== "undefined") {
  ...
  function setActivePanel(id, options = {}) {
    ...
    const panel = sliderState.panels.find((candidate) => candidate.id === id);
    syncViewportHeight(panel);
  ...
  function syncViewportHeight(panel) {
    if (!sliderState.viewport) {
      return;
    }
    const viewport = sliderState.viewport;
    const targetPanel =
      panel ||
      sliderState.panels.find((candidate) => candidate.id === activePanelId) ||
      sliderState.panels[0];
    if (!targetPanel) {
  ...
  function observeActivePanel(panel) {
    if (!panelResizeObserver) {
      return;
    }
    if (observedPanel && observedPanel !== panel) {
      panelResizeObserver.unobserve(observedPanel);
      observedPanel = undefined;
    }
    if (!panel || observedPanel === panel) {
      return;
    }
  ...
  function scrollToSection(id, options = {}) {
    ...
    const { immediate = false, updateHash = true, force = false } = options;
    if (sliderState.viewport && sliderState.panels.length > 0) {
      ...
      setActivePanel(id, { updateHash, force });
      return;
    }
    const section = document.getElementById(id);
    if (section) {
      section.scrollIntoView({ behavior: immediate ? "auto" : "smooth" });
    }
  }

(() => {
  ...
  const pngFullArtMap = new Map();
  if (typeof cardArtPngBase64 !== "undefined" && cardArtPngBase64 && typeof cardArtPngBase64 === "object") {
    for (const [cardId, base64] of Object.entries(cardArtPngBase64)) {
      ...
      pngFullArtMap.set(cardId, `data:image/png;base64,${sanitized}`);
    }
    if (typeof window !== "undefined" && typeof window.cardArtPngBase64 !== "undefined") {
      try {
        delete window.cardArtPngBase64;
      } catch (error) {
        window.cardArtPngBase64 = undefined;
      }
    }
  }
  ...
  const cardIllustrations = new Map([
    ...
    ["petal-warden", {
      image: pngFullArtMap.get("petal-warden"),
      fullCard: true,
    }],
    ["glimmer-fox", { icon: "🦊" }],
    ["embercap-shaman", {
      image: pngFullArtMap.get("embercap-shaman"),
      fullCard: true,
    }],
    ...
    ["ancient-treant", {
      image: pngFullArtMap.get("ancient-treant"),
      fullCard: true,
    }],
  ]);
  ...
  function usesFullCardArtwork(item) {
    ...
    return Boolean(artConfig?.fullCard);
  }
  ...
  function getCardArtUri(item) {
    ...
    const artConfig = cardIllustrations.get(item.id) ?? {};
    const pngOverride = pngFullArtMap.get(item.id);
    if (pngOverride) {
      cardArtCache.set(item.id, pngOverride);
      return pngOverride;
    }

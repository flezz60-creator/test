(() => {
  ...
  const cardIllustrations = new Map([
    ...
    ["petal-warden", {
      image: "https://static-cdn.stability.ai/assets/prompt-extensions/unrealistic/61.png?width=512&height=512",
    }],
    ["glimmer-fox", { icon: "🦊" }],
    ["embercap-shaman", {
      image: "https://static-cdn.stability.ai/assets/prompt-extensions/unrealistic/79.png?width=512&height=512",
    }],
    ["thornborn-stalker", { icon: "🌿" }],
    ["luminous-hart", { icon: "🦌" }],
    ["spirit-of-canopy", { icon: "🌲" }],
    ["ancient-treant", {
      image: "https://static-cdn.stability.ai/assets/prompt-extensions/unrealistic/49.png?width=512&height=512",
    }],
  ]);
  ...
  function buildDeck() {
    ...
    Object.entries(owned).forEach(([itemId, count]) => {
  ...
  function startDuel() {
    ...
    duelState.players.forEach((player) => {
      ...
      if (startingCount === 0) {
        logDuel(
          `${player.name} hat keine Karten in der Sammlung – sammle mehr Karten, um ein Match zu starten.`
        );
      } else if (startingCount < duelConfig.deckSize) {
        logDuel(
          `${player.name} startet mit ${startingCount}/${duelConfig.deckSize} Karten (${uniqueCount} einzigartig).`
        );
      } else {
        logDuel(
          `${player.name} bringt ${startingCount} Karten (${uniqueCount} einzigartig) aus seiner Sammlung ins Match.`
        );
      }
    });
    beginTurn();
  ...
  function renderDeckList() {
    ...
    players.forEach((player, index) => {
      ...
      const total = player?.startingDeck?.length ?? 0;
      const uniqueCount = new Set(player?.startingDeck ?? []).size;
      const summary = document.createElement("p");
      summary.className = "duel-decklist-summary";
      if (total === 0) {
        summary.textContent = "Keine Karten aus der Sammlung verfügbar.";
      } else {
      ...
      const entries = summarizeDeckCards(player?.startingDeck ?? []);
      if (entries.length === 0) {
        const empty = document.createElement("li");
        empty.className = "duel-decklist-empty";
        empty.textContent = "Sammle Karten, um ein vollständiges Deck zu bilden.";
        list.append(empty);
      } else {
